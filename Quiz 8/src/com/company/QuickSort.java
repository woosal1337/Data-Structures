...
class QuickSort {
    public static int numberOfEnqueue = 0;
    public static int numberOfDeque = 0;
    public static int numberOfRecursiveCalls = 0;

    public static <K> void quickSort(Queue<K> S, Comparator<K> comp) {
        // Incrementing the number of recursive operations
        ...
        while (!S.isEmpty()) {                   // divide original into L, E, and G
            K element = S.dequeue();
            // Increment the number of deque
            numberOfDeque += 1;

            int c = comp.compare(element, pivot);
            if (c < 0) {                             // element is less than pivot
                L.enqueue(element);
                // Increment the number of enqueue
                numberOfEnqueue += 1;
        ...
        while (!L.isEmpty()) {
            S.enqueue(L.dequeue());
            // Increment the number of enqueue and deque
